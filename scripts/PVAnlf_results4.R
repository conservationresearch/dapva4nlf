...
# Updated Oct 22, 2022 to reflect reviewer comments.
...
# Add that in year 0 there were 0 frogs so that the x axis starts at 0 as per reviewer request
new <- int[1,]
new$year <- 0
new$mean <- 0
new$median <- 0
...
new1 <- new
new1$alternative <- "Do Nothing"

new2 <- new
new2$alternative <- "Minimum Funding / Low Effort"

new3 <- new
new3$alternative <- "Middle of the Road"

new4 <- new
new4$alternative <- "Go Big or Go Home"

int <- rbind(int, new1, new2, new3, new4)
# int <- rbind(int, c("probability of persistence", 0, 0, 0, 0, 0, 0, 0, "Minimum Funding / Low Effort", "all populations"))
# int <- rbind(int, c("probability of persistence", 0, 0, 0, 0, 0, 0, 0, "Middle of the Road", "all populations"))
# int <- rbind(int, c("probability of persistence", 0, 0, 0, 0, 0, 0, 0, "Go Big or Go Home", "all populations"))
...
# Make the legend wider now that it is a full width graph
int3 <- results_all_iter[c(which(results_all_iter$alternative == goBig_alt_name),
                           which(results_all_iter$alternative == mostReal_alt_name),
                           which(results_all_iter$alternative == lowEffort_alt_name),
                           which(results_all_iter$alternative == doNothing_alt_name)),]
int3$alternative[which(int3$alternative == goBig_alt_name)] <- "Go Big or Go Home" 
int3$alternative[which(int3$alternative == lowEffort_alt_name)] <- "Minimum Funding /\n Low Effort" # put it on two lines
int3$alternative[which(int3$alternative == mostReal_alt_name)] <- "Middle of the Road" # put it on two lines

int3$alternative <- factor(int3$alternative, levels=c("Go Big or Go Home", "Middle of the Road",
                                                      "Minimum Funding /\n Low Effort",
                                                      "Do Nothing" )) # reorder factor levels


# Graph the excess probability distribution instead of the cumulative to show stochastic dominance
(persistence_effort_EDF1_opt2 <- dapva::graphEDF(results_summary_all_iterations = int3,
                                                 metric = "probability of persistence",
                                                 year = yrs,
                                                 # x_axis_lab = "Probability of Persistence\nin Year 50",
                                                 x_axis_lab = "Probability of Persistence in Year 50",
...
# NOW EXPORT EACH ON INDIVIDUALLY

filename <- paste("ForManuscript/graph_effort_overTime", version,".pdf", sep="")
persist_effort_graph1
ggplot2::ggsave(filename = filename, width = 6.5/2, height = 6)

filename <- paste("ForManuscript/graph_effort_violin", version,".pdf", sep="")
persistence_effort_violinPlot_opt2
ggplot2::ggsave(filename = filename, width = 6.5/2, height = 4)

filename <- paste("ForManuscript/graph_effort_excessProbDist", version,".pdf", sep="")
persistence_effort_EDF1_opt2
...
# Make further adjustments to the labels as per reviewer recommendation

tornado_parameter_labels$label[which(tornado_parameter_labels$label == "young of year\nsurvival - mean")] <-  "young of year survival rate (mean)"
tornado_parameter_labels$label[which(tornado_parameter_labels$label == "tadpole survival in\nexisting wetlands - mean")] <-  "tadpole survival rate (mean)"
tornado_parameter_labels$label[which(tornado_parameter_labels$label == "juvenile survival - mean")] <-  "juvenile survival rate (mean)"
tornado_parameter_labels$label[which(tornado_parameter_labels$label == "bullfrog mgmt. effective\n(yes = high, no = low)")] <-  "bullfrog management effective\n(yes = high, no = low)"
tornado_parameter_labels$label[which(tornado_parameter_labels$label == "adult survival - mean")] <-  "adult survival rate (mean)"
tornado_parameter_labels$label[which(tornado_parameter_labels$label == "egg survival in existing\nwetlands - mean")] <-  "egg survival rate (mean)"
tornado_parameter_labels$label[which(tornado_parameter_labels$label == "egg survival reduction if\nbullfrog mgmt. not effective")] <-  "egg survival rate reduction if bullfrog\nmanagement not effective"
tornado_parameter_labels$label[which(tornado_parameter_labels$label == "adult survival reduction\nfrom roads ")] <-  "adult survival rate reduction\nfrom road mortality "
tornado_parameter_labels$label[which(tornado_parameter_labels$label == "proportion of adults 4 years or\nolder who lay eggs - mean")] <-  "proportion of reproductive females\nage 4 years or older (mean)"
tornado_parameter_labels$label[which(tornado_parameter_labels$label == "young of year survival\nreduction from roads")] <-  "young of year survival rate reduction\n from road mortality"
...
p_hist_prob_persist_groups <- ggplot2::ggplot(test2 , ggplot2::aes(x = prob_persist, fill=group)) +
  ...
  ggplot2::xlab("Probability of Persistence in Year 50") +
  ggplot2::ylab("Number of Iterations") + 
  ggplot2::ggtitle( "") + 
...
#---- Export the histogram as a new figure for the manuscript. ----

filename <- paste("ForManuscript/graph_effort_goBig_histBimodal", version,".pdf", sep="")
p_hist_prob_persist_groups
ggplot2::ggsave(filename = filename, width = 6.5, height = 4)
