...
runAnnualLoopNLFIdahoPVA <- function(parameterByIterTracking, yrs, i, q,
  ...
  for (j in 1:yrs) { # start the annual loop
    ...
    if(bullfrogMgmt_effective == "no"){
      ...
      s_pct_reduced_juvenile_bullfrogs <- as.numeric(parameterByIterTracking[i, paste0("s_pct_reduced_juvenile_bullfrogs")])
    ...
    if(bullfrogMgmt_effective == "yes"){ # then this threat is eliminated
      ...
      s_pct_reduced_juvenile_bullfrogs <- 0
    ...
    if(drawdown_beforeMidJuly == "yes"){
      ...
      int2 <- which(wetlands_without_outside == drawdown_location_this_year)
      if(is.na(int2) == TRUE){nlf_wetland_affected <- "no"}
    ...
    if (j != 1)  { 
      ...
      # Add up the total terrestrial pop size (that is what the carrying capacity estimate is for)
      # Do this later, unlikely to get close to it for a while
